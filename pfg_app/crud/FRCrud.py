...
#     try:
#         sp = db.query(model.ServiceProvider.ServiceProviderName).filter(model.ServiceProvider.idServiceProvider == serviceaccountID).first()
#         ServiceProviderName = sp[0]
#         sps = db.query(model.ServiceProvider.idServiceProvider).filter(model.ServiceProvider.ServiceProviderName == ServiceProviderName).all()
#         count = 0
#         for v in sps:
...
#                     inputmodel["CreatedOn"] = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
...
async def getActualAccuracy(tp: str, nm: str, db):
    try:
        ...
        final_dict["DocumentCount"] = {
            "count": len(documents)
        }  # instead of int value, it should be a dict to avoid confusion and type issues
        return final_dict
    except Exception as e:
        print(e)
        return Response(status_code=500)
...
async def update_email_info(emailInfo, db):
    ...
    try:
        ...
        if emailListenerInfo is not None:
            ...
            # Initialize the email_listener_info list if it doesn't exist or is not correctly formatted
