...
def createcorpdoc(all_status, docid):
    filename = "Invoice#" + str(docid) + "_JourneyMap.pdf"
    image = "pfg_app/FROps/pfg-logo.png"
    subtitle = "Pattinson Food Group - Invoice Journey Document"
    pdf = canvas.Canvas(filename)
    ...
    for s in all_status:
        ...
        invoicetype = s.document_type
        
        doc_date = s.invoice_date
        vendor = s.VendorName
        if i == 0:
            text = pdf.beginText(40, 700)
            text.setFont("Courier-Bold", 12)
        ...
        text.setFont("Courier-Bold", 10)
        color = colors.green
        if s.corp_hist_logs.document_status in [2, 7, 14]:
            color = colors.green
        elif s.corp_hist_logs.document_status in [21]:
            color = colors.red
        else:
            color = colors.blue
        text.setFillColor(color)
        text.textLine(s.corp_hist_logs.document_desc)
        dt = s.corp_hist_logs.created_on
        pdf.drawText(text)
        text.setFillColor(colors.black)
        text.setFont("Courier", 10)
        text.textLine("User: " + s.firstName)
